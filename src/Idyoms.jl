...
function sample(dist::Distribution{T}, seed::Int) where {T}
    ...
    a = first.(ps)
    p = cumsum(last.(ps))
    n = rand(Float64) # TODO: Maybe don't take the most likely element all the time?
    q = findfirst(q -> n <= q, p)
    return first(ps[q])
end
...
function gen(seed::Int,
    ...
    nxt = sample(dist, seed)

    return Prediction(nxt, dist[nxt], ords[nxt], dist)
end

function gen_seq(seed::Int,
    len::Int,
    tally::Tally{T,T},
    seen::Set{T},
    A::Set{T},
    ...
    for i in 1:len
        ...
        p = gen(seed, ctx, tally, seen, A, B, E, U,)
        push!(preds, p)
        push!(seq, p.symbol)
    end
